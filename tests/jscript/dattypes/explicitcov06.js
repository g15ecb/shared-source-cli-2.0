...
function explicitcov06() {

    apInitTest("explicitcov06"); 
	...
	var ui11 : uint = uint(uint.MaxValue);
	sAct = ulong(ui11);
	sExp = uint.MaxValue;
	if (sAct != sExp)
	...
	var ul12 : ulong= ulong(ulong.MaxValue);
	sAct = uint(ul12);
	sExp = uint.MaxValue //4294967295;
	if (sAct != sExp)
		apLogFailInfo("12.",sExp,sAct,"");
	
	apInitScenario ("13. ushort- uint")
	var us13 : ushort= ushort(ushort.MaxValue);
	sAct = uint(us13);
	sExp = 65535;
	if (sAct != sExp)
	...
	var ui14 : uint= uint(uint.MaxValue);
	sAct = ushort(ui14);
	sExp = ushort.MaxValue;
	if (sAct != sExp)
		apLogFailInfo("14.",sExp,sAct,"");
	
	apInitScenario ("15. ushort-byte ")
	var us15: ushort= ushort(ushort.MaxValue);
	sAct = byte(us15);
	sExp = byte.MaxValue;
	if (sAct != sExp)
	...
	var ui16 : uint= uint(uint.MaxValue);
	sAct = byte(ui16);
	sExp = byte.MaxValue;
	if (sAct != sExp)
	...
	var f17: float = float(float.MaxValue);
	sAct = double(f17);
	...
	var f18: float = float(float.MaxValue);
	sAct = Number(f18);
	sExp = 3.4028234663852886e+38;
	if (sAct != sExp)
	...
	var d19: double = double(double.MaxValue);
	sAct = float(d19);
	sExp = Infinity;
	if (sAct != sExp)
	...
	var n20: Number = Number(double.MaxValue);
	sAct = float(n20);
	sExp = Infinity;
	if (sAct != sExp)
	...
	var sb21: sbyte = sbyte(sbyte.MaxValue);
	sAct = byte(sb21);
	sExp = 127;
	if (sAct != sExp)
	...
	var b21: byte = byte(byte.MaxValue);
	sAct = sbyte(sb21);
	sExp = 127;
	if (sAct != sExp)
	...
	var ui23 : uint= uint(uint.MaxValue);
	sAct = int(ui23);
	sExp = -1;
	if (sAct != sExp)
	...
	var u25: ushort= ushort(ushort.MaxValue);
	sAct = short(u25);
	sExp = -1;
	if (sAct != sExp)
	...
	var s26: short= short(short.MaxValue);
	sAct = ushort(s26);
	sExp = short.MaxValue;
	if (sAct != sExp)
	...
	var ul27 : ulong= ulong(ulong.MaxValue);
	sAct = long(ul27);
	sExp = -1 ;
	if (sAct != sExp)
	...
	var l28 : long= long(long.MaxValue);
	sAct = ulong(l28);
	sExp = long.MaxValue ;
	if (sAct != sExp)
		apLogFailInfo("27.",sExp,sAct,"");

	apInitScenario ("29. uint-long ") //bug 320953
	var ui29 : uint= uint(uint.MaxValue);
	sAct = long(ui29);
@if ( ! @_fast) 
	sExp = uint.MaxValue;
	if (sAct != sExp)
		apLogFailInfo("29.",sExp,sAct,"");
@end
	apEndTest();
}
